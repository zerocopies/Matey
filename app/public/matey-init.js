...
(function () {
  ...
  function initLicenseUI() {
    ...
    row.innerHTML='<div style="display:flex;justify-content:space-between;align-items:center;"><span style="font-size:15px;font-weight:var(--font-weight-medium);color:var(--text);">License</span><span style="font-size:13px;font-weight:var(--font-weight-medium);padding:4px 12px;border-radius:8px;'+(lic.tier==='pro'?'background:var(--accent);color:#000;':'background:var(--surface);color:var(--muted);')+'">'+(lic.tier==='pro'?'Pro':'Free')+'</span></div>'+(lic.tier==='free'?'<button id="upgrade-btn" style="border:none;background:var(--accent);color:#000;padding:10px;border-radius:12px;font:inherit;font-size:14px;font-weight:var(--font-weight-medium);cursor:pointer;">Upgrade to Pro — $5/month</button>':'<p style="font-size:12px;color:var(--muted);">Pro until '+new Date(lic.expires).toLocaleDateString()+'</p>');
    row.querySelector('#upgrade-btn')?.addEventListener('click',function(){MateyLicense.upgrade();row.remove();initLicenseUI();});card.appendChild(row);
  }
